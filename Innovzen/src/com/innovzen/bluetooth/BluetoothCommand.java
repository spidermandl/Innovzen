...
public class BluetoothCommand {
	...
	public static final int BREATHE_STOP=0x30;
	// 开机关机命令
	...
	// 10min命令
	public static int TIME10_MACHINE_VALUES[] = { START_MACHINE,
			ANDROID_TABLET, TEN_MIN, 0x11, END_MACHINE };
	// 15min命令
	public static int TIME15_MACHINE_VALUES[] = { START_MACHINE,
			ANDROID_TABLET, FIFTEEN_MIN, 0x11, END_MACHINE };
	// 20min命令
	public static int TIME20_MACHINE_VALUES[] = { START_MACHINE,
			ANDROID_TABLET, TWENTY_MIN, 0x11, END_MACHINE };
	// 25min命令
	public static int TIME25_MACHINE_VALUES[] = { START_MACHINE,
			ANDROID_TABLET, TWENTY_FIVE_MIN, 0x11, END_MACHINE };
	// 30min命令
	public static int TIME30_MACHINE_VALUES[] = { START_MACHINE,
			ANDROID_TABLET, THIRTY_MIN, 0x11, END_MACHINE };
	public static int BLUETOOTH_MACHINE_VALUES[] = { START_MACHINE,
			ANDROID_TABLET, BLUETOOTH_ON_OFF, 0x11, END_MACHINE };
	// breathe +- stop命令
	public static int BREATHE_STOP_MACHINE_VALUES[] = { START_MACHINE,
			ANDROID_TABLET, BREATHE_STOP, 0x11, END_MACHINE };
	/**
	 * 接受状态
	 */
	// 按摩椅运行状态
	...
	/**
	 * 机器所有的状态值保存在machine_status中，每次（100ms）pad接收到机器穿过来的9个字节，都会更新这个hashmap，
	 * 每个pad界面里功能按钮的状态都由machine_status决定
	...
	public void printCommand(byte[] bytes) {
		...
		for (int i = 0; i < bytes.length; i++) {
			print.append(printHX(bytes[i])).append(" ");
		}
		print.append("\r\n");
	...
	/**
	 * 解析命令 解析9个字节，并且把每个字节的状态写到machine_status
	 * 
	 * @return
	 */
	public boolean parseCommand(byte[] bytes) {
		...
		// Log.e("第3个字节", ((b2 & 0x3c) >> 2)+"~~~~~"+((b2 & 0x03) >> 0)+"``"+System.currentTimeMillis());
		 
		return true;
	}

	/*
	 * //第3字节 //第4字节 //第5字节 //第6字节 //第7个字节 byte b = bytes[6];
	...
	/*
	 * 
	 * //取第六位的状态 machine_status.put("", (b&0x40)>>5); //取第五位的状态
	 * machine_status.put("", (b&0x20)>>4); //取第三和四位的状态 machine_status.put("",
	 * (b&0x18)>>2);
	 * 
	 * 
	 * //第8字节 //第9字节 }
